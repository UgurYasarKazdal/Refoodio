...
class DeleteSelectedInventoriesUseCase(private val repository: InventoryRepository) {
    operator fun invoke(inventories: List<Int>): Flow<InventoryResource<Unit>> = flow {
        emit(repository.deleteItems(inventories))
    }.asResource()
}